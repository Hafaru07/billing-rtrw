...
// jimp v1.x mengekspor objek, bukan kelas — Jimp harus di-destructure.
// Menulis `const Jimp = require('jimp')` membuat Jimp.read undefined.
...
// Notifikasi "pembayaran berhasil" dipakai bersama portal kolektor —
// definisinya tinggal satu di services/paymentNotifyService.js supaya
// pelanggan menerima bentuk pesan yang sama dari pintu pembayaran mana pun.
const { trySendWhatsappPayment, sendPaymentSuccessWA } = require('../services/paymentNotifyService');
...
router.get('/map', requireAdminSession, requireSidebarMenuAccess('map'), (req, res) => {
  ...
  const servers = serverSvc.getAllServers();
  const poles = poleSvc.getAllPoles();
...
// ─── GRAFIK TRAFIK INTERFACE DI DASHBOARD ──────────────────────────────────
const DASHBOARD_TRAFFIC_KEY = 'dashboard_traffic_widgets';
const DASHBOARD_TRAFFIC_SLOTS = 3;

/**
 * Cache sampel trafik di sisi server.
 *
 * Tanpa ini, tiap tab dashboard yang terbuka akan menembak router sendiri-sendiri
 * setiap 2 detik. Dengan TTL 1,8 detik + dedupe in-flight, beban ke router tetap
 * konstan berapa pun jumlah admin yang membuka dashboard.
 */
const TRAFFIC_CACHE_TTL_MS = 1800;
const trafficCache = new Map();   // routerId -> { t, data }
const trafficInflight = new Map(); // routerId -> Promise

async function readRouterTrafficCached(routerId, ifaceNames) {
  const key = `${routerId ?? 'default'}|${[...ifaceNames].sort().join(',')}`;
  const now = Date.now();

  const cached = trafficCache.get(key);
  if (cached && (now - cached.t) < TRAFFIC_CACHE_TTL_MS) return cached.data;

  // Sudah ada permintaan yang sedang berjalan -> ikut menunggu, jangan tembak ulang.
  const inflight = trafficInflight.get(key);
  if (inflight) return await inflight;

  const p = (async () => {
    try {
      const rows = await mikrotikService.getInterfacesTraffic(routerId, ifaceNames);
      trafficCache.set(key, { t: Date.now(), data: rows });
      return rows;
    } finally {
      trafficInflight.delete(key);
    }
  })();

  trafficInflight.set(key, p);
  return await p;
}

function normalizeTrafficWidgets(raw) {
  const arr = Array.isArray(raw) ? raw : [];
  const out = [];
  for (let i = 0; i < DASHBOARD_TRAFFIC_SLOTS; i++) {
    const s = arr[i] || {};
    const rid = s.router_id === '' || s.router_id === null || s.router_id === undefined ? null : Number(s.router_id);
    out.push({
      router_id: Number.isFinite(rid) && rid > 0 ? rid : null,
      iface: String(s.iface || '').trim(),
...
function getTrafficWidgets() {
  return normalizeTrafficWidgets(db.getAppSetting(DASHBOARD_TRAFFIC_KEY, []));
}

// Daftar interface pada sebuah router (untuk dropdown pemilih)
router.get('/api/mikrotik/interfaces', requireAdmin, async (req, res) => {
  try {
    const rid = req.query.router_id ? Number(req.query.router_id) : null;
    const list = await mikrotikService.getInterfaces(rid && rid > 0 ? rid : null);
    res.json({ ok: true, interfaces: list });
  } catch (e) {
    res.status(200).json({ ok: false, error: e.message || 'Gagal terhubung ke router', interfaces: [] });
  }
});

// Konfigurasi 3 kotak grafik (tersimpan di DB, berlaku untuk semua admin)
router.get('/api/dashboard/traffic-widgets', requireAdmin, (req, res) => {
  try {
    const routers = (mikrotikService.getAllRouters() || []).map(r => ({ id: r.id, name: r.name, host: r.host }));
    res.json({ ok: true, widgets: getTrafficWidgets(), routers, slots: DASHBOARD_TRAFFIC_SLOTS });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});

router.post('/api/dashboard/traffic-widgets', requireAdminSession, restrictToAdmin, express.json(), (req, res) => {
  try {
    const widgets = normalizeTrafficWidgets(req.body && req.body.widgets);
    db.saveAppSetting(DASHBOARD_TRAFFIC_KEY, widgets);
    trafficCache.clear(); // paksa baca ulang dengan konfigurasi baru
    res.json({ ok: true, widgets });
  } catch (e) {
    logger.error(`[API] Simpan widget trafik gagal: ${e.message}`);
    res.status(500).json({ ok: false, error: e.message });
  }
});

/**
 * Data trafik live untuk seluruh kotak sekaligus (1 request HTTP untuk 3 kotak).
 * Interface pada router yang sama digabung agar cukup 1 koneksi RouterOS.
 */
router.get('/api/dashboard/traffic-live', requireAdmin, async (req, res) => {
  try {
    const widgets = getTrafficWidgets();

    // Kelompokkan interface per router
    const byRouter = new Map();
    widgets.forEach(w => {
      if (!w.iface) return;
      const rid = w.router_id ?? null;
      if (!byRouter.has(rid)) byRouter.set(rid, new Set());
      byRouter.get(rid).add(w.iface);
    });

    if (byRouter.size === 0) {
      return res.json({ ok: true, ts: Date.now(), slots: widgets.map(() => null) });
    }

    const results = await Promise.allSettled(
      [...byRouter.entries()].map(async ([rid, ifaces]) => ({
        rid,
        rows: await readRouterTrafficCached(rid, [...ifaces])
      }))
    );

    // Petakan hasil balik ke tiap slot
    const lookup = new Map(); // `${rid}|${iface}` -> row
    const routerError = new Map();
    results.forEach((r, idx) => {
      const rid = [...byRouter.keys()][idx];
      if (r.status === 'fulfilled') {
        r.value.rows.forEach(row => lookup.set(`${r.value.rid ?? 'default'}|${row.name}`, row));
      } else {
        routerError.set(rid ?? 'default', r.reason?.message || 'Router tidak dapat dihubungi');
      }
    });

    const slots = widgets.map(w => {
      if (!w.iface) return null;
      const rid = w.router_id ?? 'default';
      const err = routerError.get(rid);
      if (err) return { iface: w.iface, ok: false, error: err, rxMbps: 0, txMbps: 0 };
      const row = lookup.get(`${rid}|${w.iface}`);
      if (!row) return { iface: w.iface, ok: false, error: 'Data tidak tersedia', rxMbps: 0, txMbps: 0 };
      return {
        iface: row.name,
        ok: true,
        running: row.running,
        ...
        rxMbps: (Number(row.rxBps) || 0) / 1e6,
        txMbps: (Number(row.txBps) || 0) / 1e6
      };
    });

    res.json({ ok: true, ts: Date.now(), slots });
  } catch (e) {
    res.status(200).json({ ok: false, error: e.message || 'Gagal membaca trafik' });
  }
});
...
// --- SERVER / POP (PETA JARINGAN) ---
router.post('/servers', requireAdminSession, restrictToAdmin, express.urlencoded({ extended: true }), (req, res) => {
  try {
    if (!String(req.body.name || '').trim()) throw new Error('Nama server wajib diisi');
    serverSvc.createServer(req.body);
    req.session._msg = { type: 'success', text: 'Server berhasil ditambahkan.' };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal: ' + e.message };
  }
  res.redirect('/admin/map');
...
router.post('/servers/:id/update', requireAdminSession, restrictToAdmin, express.urlencoded({ extended: true }), (req, res) => {
  try {
    if (!String(req.body.name || '').trim()) throw new Error('Nama server wajib diisi');
    serverSvc.updateServer(req.params.id, req.body);
    req.session._msg = { type: 'success', text: 'Server berhasil diperbarui.' };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal: ' + e.message };
  }
  res.redirect('/admin/map');
...
router.post('/servers/:id/delete', requireAdminSession, restrictToAdmin, (req, res) => {
  try {
    serverSvc.deleteServer(req.params.id);
    req.session._msg = { type: 'success', text: 'Server berhasil dihapus. Tiang/ODP yang terhubung dilepas, tidak ikut terhapus.' };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal: ' + e.message };
  }
  res.redirect('/admin/map');
...
// --- TIANG / POLE (PETA JARINGAN) ---
router.post('/poles', requireAdminSession, restrictToAdmin, express.urlencoded({ extended: true }), (req, res) => {
  try {
    if (!String(req.body.name || '').trim()) throw new Error('Nama tiang wajib diisi');
    poleSvc.createPole(req.body);
    req.session._msg = { type: 'success', text: 'Tiang berhasil ditambahkan.' };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal: ' + e.message };
  }
  res.redirect('/admin/map');
...
router.post('/poles/:id/update', requireAdminSession, restrictToAdmin, express.urlencoded({ extended: true }), (req, res) => {
  try {
    if (!String(req.body.name || '').trim()) throw new Error('Nama tiang wajib diisi');
    poleSvc.updatePole(req.params.id, req.body);
    req.session._msg = { type: 'success', text: 'Tiang berhasil diperbarui.' };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal: ' + e.message };
  }
  res.redirect('/admin/map');
...
router.post('/poles/:id/delete', requireAdminSession, restrictToAdmin, (req, res) => {
  try {
    poleSvc.deletePole(req.params.id);
    req.session._msg = { type: 'success', text: 'Tiang berhasil dihapus. Node turunan dilepas, tidak ikut terhapus.' };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal: ' + e.message };
  }
  res.redirect('/admin/map');
...
// --- JALUR KABEL BACKBONE (Server -> Tiang -> ODP) ---
router.post('/api/poles/:id/cable-path', requireAdminSession, restrictToAdmin, (req, res) => {
  try {
    const id = Number(req.params.id);
    const { path } = req.body;
    if (!id) throw new Error('ID tiang tidak valid');
    poleSvc.updatePoleCablePath(id, path);
    res.json({ ok: true });
  } catch (e) {
    logger.error(`[API] Save Pole Cable Path Error: ${e.message}`);
    res.status(500).json({ ok: false, error: e.message });
  }
});

router.post('/api/odps/:id/cable-path', requireAdminSession, restrictToAdmin, (req, res) => {
  try {
    const id = Number(req.params.id);
    const { path } = req.body;
    ...
    logger.error(`[API] Save ODP Cable Path Error: ${e.message}`);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
router.get('/', requireAdminSession, requireSidebarMenuAccess('dashboard'), async (req, res) => {
  try {
    ...
    const trafficRouters = (mikrotikService.getAllRouters() || []).map(r => ({ id: r.id, name: r.name }));
    const trafficWidgets = getTrafficWidgets();
    res.render('admin/dashboard', {
      title: 'Dashboard', company: company(), version: '2.0.0',
      activePage: 'dashboard', billing, custStats, settings,
      trafficRouters, trafficWidgets
    });
  } catch (e) {
...
router.get('/devices', requireAdminSession, (req, res) => {
  ...
  res.render('admin/dashboard', { title: 'Monitoring ONU', company: company(), version: '2.0.0', activePage: 'devices', billing: null, custStats: null, settings, trafficRouters: [], trafficWidgets: getTrafficWidgets() });
});

router.get('/bulk', requireAdminSession, (req, res) => {
  const settings = getSettings();
  res.render('admin/dashboard', { title: 'Konfigurasi Massal', company: company(), version: '2.0.0', activePage: 'bulk', billing: null, custStats: null, settings, trafficRouters: [], trafficWidgets: getTrafficWidgets() });
});
...
const ADMIN_PRINT_CTX = { basePath: '/admin/billing', backUrl: '/admin/billing' };

router.get('/billing/:id/print', requireAdminSession, (req, res) =>
  invoicePrint.renderPrintPage(req, res, ADMIN_PRINT_CTX));

/**
 * Data cetak ESC/POS untuk printer thermal — dipakai halaman struk termal
 * untuk mengirim struk ke RawBT tanpa dialog cetak sistem.
 */
router.get('/billing/:id/escpos', requireAdminSession, invoicePrint.sendEscpos);
...
router.post('/billing/:id/qris-assign', requireAdminSession, (req, res) => {
  try {
    ...
    const picked = pickQrisUniqueAmount(baseAmount, (amount) => !exists.get('unpaid', amount, invId));
    if (!picked) throw new Error(qrisRangeFullMessage());

    update.run(picked.code, picked.amount, invId);
    req.session._msg = { type: 'success', text: `Kode QRIS dibuat: Rp ${Number(picked.amount).toLocaleString('id-ID')} (kode ${picked.code}).` };
  } catch (e) {
  ...
  try {
    ...
    if (hasStaticQris && String(inv.status) === 'unpaid' && (!qrisAmountUnique || !qrisCode)) {
      ...
      if (Number.isFinite(invId) && invId > 0 && Number.isFinite(baseAmount) && baseAmount > 0) {
        ...
        if (picked) {
          update.run(picked.code, picked.amount, invId);
          qrisAmountUnique = picked.amount;
          qrisCode = picked.code;
        } else {
          logger.warn(`[QRIS] Slot kode unik penuh untuk invoice ${invId} (nominal ${baseAmount}).`);
        }
      }
    }
    ...
    if (isQrisCase) {
      try {
        ...
        if (!payloadNorm) {
          logger.warn('[QRIS] Payload merchant tidak terbaca — isi "Payload QRIS" di Pengaturan atau unggah gambar QRIS yang jelas. Tagihan dikirim sebagai teks.');
        } else {
          const dynamic = convertStaticQrisToDynamic(payloadNorm, qrisAmountUnique);
          // PNG langsung dari QRCode: lossless (lebih andal dipindai) & jauh
          // lebih kecil daripada hasil konversi JPEG.
          const png = await QRCode.toBuffer(dynamic, { errorCorrectionLevel: 'M', margin: 2, width: 512, type: 'png' });
          sent = await sendWAImage(customer.phone, png, qrisJpgCaption);
          if (!sent) logger.warn(`[QRIS] sendWAImage gagal untuk ${customer.phone}, jatuh ke pesan teks.`);
        }
      } catch (e) {
        // Sebelumnya error di sini ditelan diam-diam, sehingga kegagalan
        // pembuatan QR tidak pernah terlihat di log.
        logger.error(`[QRIS] Gagal menyiapkan QR untuk ${customer.name}: ${e.message}`);
        sent = false;
      }
    }
    if (!sent) {
...
router.get('/reports/print', requireAdminSession, requireSidebarMenuAccess('reports'), (req, res) => {
  ...
  // Rincian pemasukan agar laporan laba/rugi seimbang — sebelumnya hanya
  // pengeluaran yang dirinci, pemasukan tampil sebagai satu angka gelondongan.
  const incomeByCategory = [
    { category: 'Pembayaran Tagihan Pelanggan', total: revenueYearDirect },
    { category: 'Deposit / Top-up Agen', total: agentDepositYear },
    { category: 'Kas Masuk Lain-lain', total: customCashInYear }
  ].filter(r => Number(r.total) > 0).sort((a, b) => b.total - a.total);
...
// Upload tanda tangan digital untuk invoice (disimpan sebagai public/img/signature.png)
router.post('/settings/signature-upload', requireAdminSession, restrictToAdmin, qrisUpload.single('signature_file'), async (req, res) => {
  try {
    const f = req.file;
    if (!f || !f.buffer || !f.originalname) throw new Error('File tanda tangan tidak ditemukan');

    const ext = String(path.extname(f.originalname || '') || '').toLowerCase();
    const allowedExt = new Set(['.png', '.webp']);
    const allowedMime = new Set(['image/png', 'image/webp']);
    if (!allowedExt.has(ext) || !allowedMime.has(String(f.mimetype || '').toLowerCase())) {
      throw new Error('Gunakan PNG atau WebP dengan latar transparan');
    }

    const dir = path.join(__dirname, '../public/img');
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'signature.png'), f.buffer);

    req.session._msg = { type: 'success', text: 'Tanda tangan digital berhasil diperbarui!' };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal upload tanda tangan: ' + (e?.message || e) };
  }
  res.redirect('/admin/settings');
});
...
router.post('/settings', requireAdminSession, express.urlencoded({ extended: true }), (req, res) => {
  try {
    ...
    // Rentang kode unik QRIS — simpan sebagai angka & jaga tetap masuk akal
    if (newSettings.qris_unique_min !== undefined) {
      let v = parseInt(newSettings.qris_unique_min, 10);
      newSettings.qris_unique_min = (Number.isFinite(v) && v >= 1) ? Math.min(v, 999) : 1;
    }
    if (newSettings.qris_unique_max !== undefined) {
      let v = parseInt(newSettings.qris_unique_max, 10);
      newSettings.qris_unique_max = (Number.isFinite(v) && v >= 1) ? Math.min(v, 999) : 20;
    }
    if (newSettings.qris_unique_min > newSettings.qris_unique_max) {
      const tmp = newSettings.qris_unique_min;
      newSettings.qris_unique_min = newSettings.qris_unique_max;
      newSettings.qris_unique_max = tmp;
    }
    if (newSettings.invoice_ppn_percentage !== undefined) {
      const v = parseFloat(newSettings.invoice_ppn_percentage);
      newSettings.invoice_ppn_percentage = (Number.isFinite(v) && v >= 0) ? v : 12;
    }
