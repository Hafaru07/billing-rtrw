  ...
  try {
    ...
    if (payload) {
      try {
        ...
        const jpg = png; // PNG langsung: lossless, jauh lebih kecil, tanpa langkah Jimp
...
function ensureInvoiceQrisUnique(inv, force = false) {
  ...
  const picked = pickQrisUniqueAmount(baseAmount, (amount) =>
    isQrisAmountAvailable(amount, { excludeInvoiceId: invId })
  );
  if (!picked) throw new Error(qrisRangeFullMessage());

  update.run(picked.code, picked.amount, invId);
  return { uniqueCode: picked.code, amountUnique: picked.amount };
}
...
function ensureVoucherOrderQrisUnique(order, force = false) {
  ...
  const picked = pickQrisUniqueAmount(baseAmount, (amount) =>
    isQrisAmountAvailable(amount, { excludeVoucherOrderId: orderId })
  );
  if (!picked) throw new Error(qrisRangeFullMessage());

  update.run(picked.code, picked.amount, orderId);
  return { uniqueCode: picked.code, amountUnique: picked.amount };
}
