...
// ─── INTERFACE & TRAFIK REALTIME ───────────────────────────────────────────

/** Ambil angka dari objek RouterOS yang nama fieldnya bisa camelCase atau kebab-case. */
function ifNum(obj, keys) {
  if (!obj) return 0;
  for (const k of keys) {
...
function ifBool(obj, keys) {
  const v = ifStr(obj, keys);
  return v === 'true' || v === 'yes' || v === '1';
}

/** Jalankan perintah menu RouterOS lintas versi library. */
async function runMenuCommand(menu, command, args) {
  if (!menu) return null;
...
/**
 * Daftar interface pada router. Di-cache 30 detik karena daftar interface
 * jarang berubah — yang sering berubah hanya angka trafiknya.
 */
async function getInterfaces(routerId = null) {
  ...
  try {
    ...
    const list = (Array.isArray(rows) ? rows : [])
      .map(r => ({
        name: ifStr(r, ['name']),
        type: ifStr(r, ['type']),
        comment: ifStr(r, ['comment']),
        running: ifBool(r, ['running']),
        disabled: ifBool(r, ['disabled'])
      }))
      .filter(r => r.name);
    setCachedList(ck, list);
    return list;
  } catch (e) {
    logger.error(`[MikroTik] Gagal ambil daftar interface: ${e.message}`);
    throw e;
  } finally {
    if (conn && conn.api) conn.api.close();
  }
...
// Penyimpan sampel byte-counter untuk mode fallback (kalau monitor-traffic tidak tersedia).
const ifTrafficSamples = new Map();

/**
 * Baca trafik beberapa interface sekaligus dalam SATU koneksi.
 *
 * Penting untuk beban router: memanggil fungsi ini dengan 3 interface hanya
 * membuka 1 koneksi API, bukan 3. getConnection() tidak melakukan pooling,
 * jadi menggabungkan seperti ini jauh lebih hemat.
 *
 * Utama memakai `/interface monitor-traffic once` yang langsung memberi
 * bit-per-detik. Bila tidak didukung, jatuh ke selisih rx-byte/tx-byte
 * antar pemanggilan.
 *
 * @param {number|string|null} routerId
 * @param {string[]} names daftar nama interface
 * @returns {Promise<Array<{name:string,rxBps:number,txBps:number,running:boolean,source:string,warmup:boolean}>>}
 */
async function getInterfacesTraffic(routerId = null, names = []) {
  ...
  try {
    ...
    // Sekali baca untuk status running + byte counter semua interface.
    let rowsByName = new Map();
    try {
      const rows = await ifMenu.get();
      for (const r of (Array.isArray(rows) ? rows : [])) {
        const nm = ifStr(r, ['name']);
        if (nm) rowsByName.set(nm, r);
      }
    } catch (e) { /* lanjut, status running jadi tidak diketahui */ }

    const now = Date.now();
    const out = [];

    for (const name of wanted) {
      const row = rowsByName.get(name) || null;
      const running = row ? ifBool(row, ['running']) : true;
      const disabled = row ? ifBool(row, ['disabled']) : false;

      if (!row) {
        out.push({ name, rxBps: 0, txBps: 0, running: false, source: 'not-found', warmup: false, error: 'interface tidak ditemukan' });
        continue;
      }
      if (disabled || !running) {
        out.push({ name, rxBps: 0, txBps: 0, running: false, source: 'down', warmup: false });
        continue;
      }

      // 1) monitor-traffic once -> bit per detik langsung
      let got = false;
      try {
        const raw = await runMenuCommand(ifMenu, 'monitor-traffic', { interface: name, once: '' });
        const mt = Array.isArray(raw) ? raw[0] : raw;
        const rxBps = ifNum(mt, ['rxBitsPerSecond', 'rx-bits-per-second']);
        const txBps = ifNum(mt, ['txBitsPerSecond', 'tx-bits-per-second']);
        if (mt && (rxBps || txBps || rxBps === 0)) {
          out.push({ name, rxBps, txBps, running: true, source: 'monitor-traffic', warmup: false });
          got = true;
        }
      } catch (e) { /* fallback di bawah */ }
      if (got) continue;

      // 2) Fallback: selisih byte counter antar pemanggilan
      const rxByte = ifNum(row, ['rxByte', 'rx-byte', 'rxBytes', 'rx-bytes']);
      const txByte = ifNum(row, ['txByte', 'tx-byte', 'txBytes', 'tx-bytes']);
      const key = `${routerId || 'default'}:${name}`;
      const prev = ifTrafficSamples.get(key);
      ifTrafficSamples.set(key, { t: now, rxByte, txByte });

      if (!prev || !prev.t || now <= prev.t) {
        out.push({ name, rxBps: 0, txBps: 0, running: true, source: 'counter', warmup: true });
        continue;
      }

      const dt = (now - prev.t) / 1000;
      // Counter mundur = router restart / counter reset -> anggap sampel baru.
      const dRx = rxByte >= prev.rxByte ? rxByte - prev.rxByte : 0;
      const dTx = txByte >= prev.txByte ? txByte - prev.txByte : 0;
      out.push({
        name,
        rxBps: dt > 0 ? (dRx * 8) / dt : 0,
        txBps: dt > 0 ? (dTx * 8) / dt : 0,
        running: true,
        source: 'counter',
        warmup: false
      });
