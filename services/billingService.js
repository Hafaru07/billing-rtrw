...
/**
 * Pengaman: pastikan tagihan bulan berjalan sudah dibuat.
 *
 * Cron generate tagihan hanya menyala sekali (tanggal 1 pukul 00:01) dan
 * node-cron tidak menjalankan jadwal yang terlewat. Kalau server mati atau
 * restart tepat di momen itu, tagihan sebulan tidak pernah dibuat — akibatnya
 * pengingat WhatsApp dan isolir otomatis ikut tidak jalan tanpa error apa pun.
 *
 * Fungsi ini aman dipanggil berulang: generateMonthlyInvoices() melewati
 * pelanggan yang invoice periodenya sudah ada, jadi tidak akan dobel.
 *
 * @returns {{ month:number, year:number, created:number, alreadyComplete:boolean }}
 */
function ensureCurrentMonthInvoices() {
  const now = getCurrentDateInTimezone();
  const month = now.getMonth() + 1;
  const year = now.getFullYear();

  const eligible = db.prepare(
    "SELECT COUNT(*) as c FROM customers WHERE status IN ('active','suspended') AND package_id IS NOT NULL"
  ).get();
  const existing = db.prepare(
    'SELECT COUNT(*) as c FROM invoices WHERE period_month=? AND period_year=?'
  ).get(month, year);

  const eligibleCount = Number(eligible?.c) || 0;
  const existingCount = Number(existing?.c) || 0;

  if (eligibleCount === 0 || existingCount >= eligibleCount) {
    return { month, year, created: 0, alreadyComplete: true };
  }

  const created = generateMonthlyInvoices(month, year);
  return { month, year, created, alreadyComplete: false };
}
