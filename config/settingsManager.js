...
/**
 * settings.json sengaja TIDAK diikutkan ke git (berisi password & API key),
 * sehingga hasil `git clone` yang masih baru belum memilikinya. Tanpa berkas
 * itu aplikasi gagal jalan dan pesan errornya membingungkan.
 *
 * Di sini disalin otomatis dari settings.example.json bila belum ada, supaya
 * aplikasi tetap bisa start dan admin tinggal mengisi lewat halaman Pengaturan.
...
function ensureSettingsFile() {
  try {
    ...
    logger.warn('[settings] settings.json belum ada — disalin dari settings.example.json. ' +
                'Segera ubah password admin, session_secret, dan kredensial lain di menu Pengaturan.');
  } catch (e) {
    logger.error(`[settings] Gagal menyiapkan settings.json: ${e.message}`);
  }
}
ensureSettingsFile();
