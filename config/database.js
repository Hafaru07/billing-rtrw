...
db.exec(`
  ...
  -- Penanda pengingat tagihan WhatsApp yang sudah dikirim, per pelanggan per hari.
  -- UNIQUE(customer_id, reminder_date) membuat proses kirim bersifat idempoten:
  -- setelah restart, pelanggan yang sudah ditandai 'sent' tidak dikirimi ulang.
  CREATE TABLE IF NOT EXISTS billing_reminder_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    customer_id INTEGER NOT NULL REFERENCES customers(id) ON DELETE CASCADE,
    reminder_date TEXT NOT NULL,              -- 'YYYY-MM-DD' waktu lokal
  ...
  CREATE INDEX IF NOT EXISTS idx_reminder_logs_date ON billing_reminder_logs(reminder_date);

  -- Titik server / POP / backbone di peta jaringan (root topologi)
  CREATE TABLE IF NOT EXISTS servers (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    type TEXT DEFAULT 'pop',          -- pop, olt, router, backbone, bts
    lat TEXT,
    lng TEXT,
    address TEXT DEFAULT '',
    capacity INTEGER NOT NULL DEFAULT 0, -- kapasitas core/port keluar (0 = tak dibatasi)
    description TEXT DEFAULT '',
    is_active INTEGER DEFAULT 1,
    created_at DATETIME DEFAULT (NOW_LOCAL())
  );

  -- Tiang / pole sebagai titik lintasan kabel (Server -> Tiang -> Tiang -> ODP)
  CREATE TABLE IF NOT EXISTS poles (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    type TEXT DEFAULT 'beton',        -- beton, besi, kayu, existing (numpang)
    parent_type TEXT DEFAULT '',      -- '', 'server', 'pole'
    parent_id INTEGER,
    lat TEXT,
    lng TEXT,
    cable_path TEXT,                  -- polyline JSON jalur kabel ke parent
    description TEXT DEFAULT '',
    created_at DATETIME DEFAULT (NOW_LOCAL())
  );
...
// Kolom topologi peta jaringan: ODP menempel ke server / tiang + jalur kabel uplink
...
try { db.exec("ALTER TABLE packages ADD COLUMN ppn_percentage REAL DEFAULT 12.0"); } catch (e) {}
